...
def arima_rD(rD,rA,df):
    ...
    for param in pdq:
        try:
            model_india = ARIMA(df1["detailed"][0:97], order=param)
            model_india_fit = model_india.fit()
            india_predictions = model_india_fit.forecast(steps=11)[0]
            rmse.append(math.sqrt(mean_squared_error(df1["detailed"][97:],india_predictions)))
            if rmse[-1] == min(rmse):
                parameter = param
        except:
            continue
    ...
    #ARIMA
    arima_values = ARIMA(df1["detailed"][:97],(8,2,0)).fit()
    #print(arima_values.summary())
    
    prediction = arima_values.forecast(steps = 11)[0]
    residual_arima = df1["detailed"][97:] - prediction
